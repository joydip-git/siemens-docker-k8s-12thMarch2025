...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`http://${AUTH_SERVICE}:${PORT_NUMBER}/hashed-password/${password}`);
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${AUTH_SERVICE_SERVICE_HOST}:${PORT_NUMBER}/token/${hashedPassword}/${password}`);
